...
########################################################################################
    ...
    def drive_mode(self) -> int:
        ...
        if self.car_1 and self.lane_state == 1:
            ...
            # 2차선에 차량이 있는 경우
            elif len(self.car_gap_reg) >= 3:
                diff = self.car_gap_reg[-1] - self.car_gap_reg[0]
                gap = self.car_data.y[self.car_location_data.index(1)] - self.car_data.y[self.car_location_data.index(2)]
                        
                # 두 차량이 차선을 막고 있는 경우
                if self.car_gap_reg[-1] < 5: 
                    self.get_logger().info(f"DEBUG 1-0")

                    # 정지
                    return 3
                
                # 두 차량 간의 거리가 증가하고 1차선 차량이 가깝게 위치한 경우
                elif diff > 0 and  gap > 0:
                    self.get_logger().info(f"DEBUG 1-1")       
                    
                    # 차선 변경
                    return 2

                # 두 차량 간의 거리가 증가하고 2차선 차량이 가깝게 위치한 경우
                elif diff > 0 and  gap < 0:
                    self.get_logger().info(f"DEBUG 1-2") 

                    # 서행 및 직진
                    self.send_command(steer_angle = steer_angle, left_speed = 100, right_speed = 100) 
                    return 1

                # 두 차량 간의 거리가 감소하고 1차선 차량이 가깝게 위치한 경우
                elif diff < 0 and  gap > 0:
                    self.get_logger().info(f"DEBUG 1-3") 

                    # 서행 및 직진
                    self.send_command(steer_angle = steer_angle, left_speed = 100, right_speed = 100) 
                    return 1

                # 두 차량 간의 거리가 감소하고 2차선 차량이 가깝게 위치한 경우
                elif diff < 0 and  gap < 0:
                    self.get_logger().info(f"DEBUG 1-4")
        ...
        if self.car_2 and self.lane_state == 2:
            ...
            # 1차선에 차량이 있는 경우
                ...
                # 두 차량이 차선을 막고 있는 경우
                if self.car_gap_reg[-1] < 5:
                    self.get_logger().info(f"DEBUG 2-0")
                ...
                # 두 차량 간의 거리가 증가하고 1차선 차량이 가깝게 위치한 경우
                    ...
                    self.get_logger().info(f"DEBUG 2-1")
                ...
                # 두 차량 간의 거리가 증가하고 2차선 차량이 가깝게 위치한 경우
                    ...
                    self.get_logger().info(f"DEBUG 2-2")
                ...
                # 두 차량 간의 거리가 감소하고 1차선 차량이 가깝게 위치한 경우
                    ...
                    self.get_logger().info(f"DEBUG 2-3")
                ...
                # 두 차량 간의 거리가 감소하고 2차선 차량이 가깝게 위치한 경우
                    ...
                    self.get_logger().info(f"DEBUG 2-4")
...
########################################################################################
    ...
    def lane_change_mode(self) -> int:
        ...
        if self.lane_state == 2:
            ...
            # 조향 각도 제한 [좌:-40, 우:40]
            self.send_command(steer_angle = int(np.clip(steer_angle, -40, 40)), left_speed = 200, right_speed = 200) 
            
            # Driving Mode로의 변동 조건 (Count 30 이상)
            if self.cnt > 30:
                self.lane_state = 1
                self.cnt = 0
                return 1
        ...
        if self.lane_state == 1:
            ...
            # 조향 각도 제한 [좌:-40, 우:40]
            self.send_command(steer_angle = int(np.clip(steer_angle, -40, 20)), left_speed = 200, right_speed = 200) 
...
########################################################################################
    ...
    def stop_mode(self) -> int:
        ...
        # 주행 방해 요건이 없고 신호등이 빨간색이 아닐 경우
        if self.car_gap_reg[-1] >= 5 and self.traffic_data.data != "R":
            return 1
